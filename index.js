...
io.on('connection', (socket) => {
  socket.on('newPlayer', (playerName) => {
    ...
    gameState.players[socket.id] = {
      x: 250,
      y: 250,
      name: playerName || 'Anonymous' // Set player name or default to 'Anonymous'
    };
  });
  ...
  socket.on("disconnect", () => {
    // When someone leaves, remove them from the game state
    delete gameState.players[socket.id];
  });
});

// Emit the game state to the clients 60 times / second
setInterval(() => {
